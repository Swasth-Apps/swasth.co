...
class ProfessionalPrograms extends React.Component {
    constructor(props) {
        ...
        const organizationId = urlParams?.get('uuid');
        this.state = {
            programs: [],
            organizationId,
        }
    ...
    componentDidMount() {
        ...
        if(organizationId && !this.props.organization?.[organizationId]){
            this.props.setMarketingLoading(true);

            API.graphql(graphqlOperation(getOrganizationWithMembersAndPrograms,{organizationId}), {
                "x-api-key": graphql_endpoint.COACHING_API_KEY
            }).then(({data}) => {
                this.props.setOrganizationById(data?.getOrganizationWithMembersAndPrograms,organizationId);
    ...
    render() {
        ...
        return (this.state.organizationId ?
                <ProfessionalPage organization={this.props.organization} organizationId={this.state.organizationId}/> :
                <>
                        ...
                        {coachingLoading ?
                            <div className="clients-navigation-wrapper">
                                ...
                                <div className="coach-grid-view">
                                    <Link to={`/coaching/professionals/${org.name?.split(" ")?.join("-").toLowerCase()}?uuid=${org.id}`} className="coach-grid-view-card">
